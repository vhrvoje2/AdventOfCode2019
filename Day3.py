...
wire1List = returnCoords(wire1)
wire2List = returnCoords(wire2)

wire1Set = set()
wire2Set = set()

for element in wire1List:
    if element != (0, 0):
        wire1Set.add(element)

for element in wire2List:
    if element != (0, 0):
        wire2Set.add(element)

matches = wire1Set.intersection(wire2Set)

listDist = []
for element in matches:
    listDist.append(abs(element[0]) + abs(element[1]))

print("Solution part 1: " + str(min(listDist)))
    ...
    for step in steps:
        ...
        if moveDir == 'R':
            for _ in range(0, moveAmt):
                ...
                moves += 1
                listCoords.append((currX, currY))
                if (currX, currY) in matches:
                    if (currX, currY) not in dictSteps:
                        dictSteps[(currX, currY)] = moves
                    else:
                        dictSteps[(currX, currY)] += moves
...
print(dictSteps[min(dictSteps, key=dictSteps.get)])